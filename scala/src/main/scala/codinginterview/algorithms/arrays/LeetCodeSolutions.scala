...
object LeetCodeSolutions {
  ...
  // period '..') Return the simplified canonical path.
  def simplifyPath(path: String): String = {
    var directoryStack: Stack[String] = new Stack()

    var directories = path.split("/").filter(!_.isEmpty())

    for (dir <- directories) {
      if (dir == ".." && !directoryStack.isEmpty) {
        directoryStack.pop()
      } else if (dir != ".") {
    ...
    while (!directoryStack.isEmpty) {
      fullPath = "/" + directoryStack.pop() + fullPath
    }
